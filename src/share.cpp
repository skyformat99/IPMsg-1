...
BOOL ShareMng::SetFileInfo(char *fname, FileInfo *info)
{
	...
	if (GetFileInfomation(fname, &fdat, FALSE) != TRUE)
		return	FALSE;
...
BOOL TShareDlg::EvCreate(LPARAM lParam)
{
	...
	char	*title[] = { GetLoadStr(IDS_FILENAME), GetLoadStr(IDS_SIZE), GetLoadStr(IDS_LOCATION), NULL };
	int	size[] = { 120, 70, 180 };
...
BOOL TShareDlg::FileAddDlg(TDlg *dlg, ShareMng *shareMng, ShareInfo *shareInfo, Cfg *cfg)
{
	...
	if (OpenFileDlg(dlg, OpenFileDlg::MULTI_OPEN).Exec(buf, GetLoadStr(IDS_ADDFILE), GetLoadStrAsFilter(IDS_OPENFILEALLFLTR), cfg->lastOpenDir) != TRUE)
		return	FALSE;
...
int TSaveCommonDlg::Exec(void)
{
	...
	// ファイルダイアログ
		...
		BOOL	ret = OpenFileDlg(parentWin, OpenFileDlg::NODEREF_SAVE, (LPOFNHOOKPROC)TApp::WinProc).Exec(fname, GetLoadStr(IDS_SAVEFILE), GetLoadStrAsFilter(IDS_OPENFILEALLFLTR), last_dir);
	...
	// shortcut の場合は、リンク先に飛ぶ
		if (!isLinkFile && (ext = strrchr(fname, '.')) && stricmp(ext, ".lnk") == 0) {
			char	arg[MAX_BUF];
			if (ReadLink(fname, last_dir, arg)) {
	...
	// 上書き確認
		for (int cnt=0; cnt < shareInfo->fileCnt; cnt++)
		{
			...
			{
				if ((ret = parentWin->MessageBox(GetLoadStr(IDS_OVERWRITE), GetLoadStr(IDS_ATTENTION), MB_OKCANCEL|MB_ICONEXCLAMATION) == IDOK) == FALSE)
...
// ボタン高さの2倍分広げる
	...
	CreateWindowU8(BUTTON_CLASS, GetLoadStr(IDS_PREVBUTTON), flg | BS_PUSHBUTTON, cx, cy, ok_xsize, ok_ysize, hWnd, (HMENU)PRIOR_BUTTON, TApp::GetInstance(), NULL);
	CreateWindowU8(BUTTON_CLASS, GetLoadStr(IDS_NEXTBUTTON), flg | BS_PUSHBUTTON, cx+=ok_xsize+20, cy, ok_xsize, ok_ysize, hWnd, (HMENU)NEXT_BUTTON, TApp::GetInstance(), NULL);
...
BOOL TSaveCommonDlg::SetInfo(void)
{
	...
	if (GET_MODE(shareInfo->fileInfo[offset]->Attr()) == IPMSG_FILE_DIR)
		strcpy(sizestr, GetLoadStr(IDS_DIRSAVE));
	else
		MakeSizeString(sizestr, shareInfo->fileInfo[offset]->Size());

	wsprintf(buf, GetLoadStr(IDS_FILEINFO), offset + 1, shareInfo->fileCnt, shareInfo->fileInfo[offset]->Fname(), sizestr);
	...
	wsprintf(buf, GetLoadStr(IDS_LUMPBUTTON), sizestr, shareInfo->fileCnt);
	SetDlgItemText(LUMP_CHECK, buf);

	char	*ext = strrchr(shareInfo->fileInfo[offset]->Fname(), '.');
...
ShareInfo *DecodeShareMsg(char *buf)
{
	...
	for (int cnt=0; file; cnt++, file=separate_token(NULL, FILELIST_SEPARATOR, &p))
	{
		...
		if ((tok = separate_token(NULL, ':', &p2)) == NULL || strlen(tok) >= MAX_FILENAME)
			break;
		while ((p3 = strchr(tok, '?')) != NULL)	// UNICODE 対応までの暫定
			*p3 = '_';
...
BOOL GetFileInfomation(const char *path, WIN32_FIND_DATA_U8 *fdata, BOOL need_fname)
{
	HANDLE	fh;
