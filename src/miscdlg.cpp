...
BOOL TAboutDlg::EvCommand(WORD wNotifyCode, WORD wID, LPARAM hWndCtl)
{
	...
	case IPMSG_ICON: case IPMSGWEB_BUTTON:
		if (wID == IPMSGWEB_BUTTON || wNotifyCode == 1)
			ShellExecuteU8(NULL, NULL, GetLoadStr(IDS_IPMSGURL), NULL, NULL, SW_SHOW);
		return	TRUE;
	}
	return	FALSE;
}
...
BOOL TEditSub::CreateByWnd(HWND _hWnd)
{
	if (!TSubClassCtl::CreateByWnd(_hWnd))
		return	FALSE;
...
BOOL TEditSub::EvContextMenu(HWND childWnd, POINTS pos)
{
	...
	AppendMenuU8(hMenu, MF_STRING|((is_readonly || !SendMessage(EM_CANUNDO, 0, 0)) ? MF_DISABLED|MF_GRAYED : 0), WM_UNDO, GetLoadStr(IDS_UNDO));
	AppendMenuU8(hMenu, MF_SEPARATOR, 0, 0);
	AppendMenuU8(hMenu, MF_STRING|(is_readonly ? MF_DISABLED|MF_GRAYED : 0), WM_CUT, GetLoadStr(IDS_CUT));
	AppendMenuU8(hMenu, MF_STRING, WM_COPY, GetLoadStr(IDS_COPY));
	AppendMenuU8(hMenu, MF_STRING|((is_readonly || !SendMessage(EM_CANPASTE, 0, 0)) ? MF_DISABLED|MF_GRAYED : 0), WM_PASTE, GetLoadStr(IDS_PASTE));
	AppendMenuU8(hMenu, MF_STRING|(is_readonly ? MF_DISABLED|MF_GRAYED : 0), WM_CLEAR, GetLoadStr(IDS_DELETE));
	AppendMenuU8(hMenu, MF_SEPARATOR, 0, 0);
	AppendMenuU8(hMenu, MF_STRING, EM_SETSEL, GetLoadStr(IDS_SELECTALL));
		...
		if (start != end && end < MAX_UDPBUF) {
			...
			if (obj && *obj->program) {
				if ((int)ShellExecuteU8(NULL, NULL, obj->program, url_ptr ? url_ptr : u8buf, NULL, SW_SHOW) <= WINEXEC_ERR_MAX)
					MessageBox(obj->program, GetLoadStr(IDS_CANTEXEC), MB_OK|MB_ICONINFORMATION);
			}
...
{
   memcpy(pbBuff,(void*)dwCookie,cb);
...
//@@int TEditSub::ExGetRtfText(void *buf,BOOL bRtfFormat/*=TRUE*/)
//@@{
//@@    EDITSTREAM es={0};
//@@    es.dwCookie = (DWORD) buf;
//@@    es.pfnCallback = MyStreamOutCallback; 
//@@ 
//@@	return SendMessageW(EM_STREAMOUT,bRtfFormat?SF_RTF:SF_TEXT,LPARAM(&es));
//@@}
//@@
//@@BOOL TEditSub::ExSetRtfText(const void *buf,BOOL bRtfFormat/*=TRUE*/)
//@@{
//@@    EDITSTREAM es={0};
//@@    es.dwCookie = (DWORD) buf;
//@@    es.pfnCallback = MyStreamInCallback; 
//@@ 
//@@	return SendMessageW(EM_STREAMIN,bRtfFormat?SF_RTF:SF_TEXT,LPARAM(&es));
//@@
//@@}
	...
	case ABSENCE_LIST:
		if (wNotifyCode == LBN_SELCHANGE)
		{
			...
			if ((index = (int)SendDlgItemMessage(ABSENCE_LIST, LB_GETCURSEL, 0, 0)) != LB_ERR)
			{
				...
				GetDlgItemText(ABSENCE_EDIT, tmpAbsenceStr[currentChoice], MAX_PATH_U8);
...
void TAbsenceDlg::SetData(void)
{
	for (int cnt=0; cnt < cfg->AbsenceMax; cnt++)
	{
		...
		Wstr	head_w(cfg->AbsenceHead[cnt]);
	...
	case ADD_BUTTON:
		{
			...
			{
				wsprintf(buf, GetLoadStr(IDS_EXECPROGRAM), protocol);
	...
	case IDOK:
		{
			...
			if (cfg)
			{
				if (CheckPassword(cfg->PasswordStr, buf))
					...
					SetDlgItemText(PASSWORD_EDIT, ""), MessageBox(GetLoadStr(IDS_CANTAUTH));
			}
			else EndDialog(TRUE);
		}
		return	TRUE;
	...
	case IDOK:
		...
		if (CheckPassword(cfg->PasswordStr, buf))
		{
			...
			if (strcmp(buf, buf2) == 0)
				...
				return	MessageBox(GetLoadStr(IDS_NOTSAMEPASS)), TRUE;
			cfg->WriteRegistry(CFG_GENERAL);
			EndDialog(TRUE);
		}
		else
