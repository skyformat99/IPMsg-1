...
char	*SetupFiles [] = { INSTALL_EXENAME, README_NAME, NULL };
...
{ 
	...
	parentWin->GetDlgItemText(editCtl, buf, sizeof(buf));
...
BOOL TInputDlg::EvCommand(WORD wNotifyCode, WORD wID, LPARAM hwndCtl)
{
	...
	{
	case IDOK:
		GetDlgItemText(INPUT_EDIT, dirBuf, MAX_PATH_U8);
		EndDialog(TRUE);
		return	TRUE;
