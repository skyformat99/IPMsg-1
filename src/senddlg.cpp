...
*/
	...
	hostListHeader.CreateByWnd((HWND)hostListView.SendMessage(LVM_GETHEADER, 0, 0));
	...
	if (cfg->AbnormalButton)
		SetDlgItemText(IDOK, GetLoadStr(IDS_FIRE));
...
BOOL TSendDlg::BallonWindow(LPCSTR msg, LPCSTR title)
{
	NOTIFYICONDATA2W	tn;
...
BOOL TSendDlg::EvCommand(WORD wNotifyCode, WORD wID, LPARAM hWndCtl)
{
	...
	case SEND_MSG:case IDOK:
		...
		{
			if (MessageBox(GetLoadStr(IDS_ABSENCEQUERY), MSG_STR, MB_OKCANCEL) != IDOK)
				return	TRUE;
	...
	case MENU_FOLDERADD:
		{
			...
			if (BrowseDirDlg(this, GetLoadStr(IDS_FOLDERATTACH), cfg->lastOpenDir, cfg->lastOpenDir))
			{
	...
	default:
		if (wID >= MENU_PRIORITY_RESET && wID < MENU_GROUP_START)
		{
			...
			{
				if (MessageBox(GetLoadStr(IDS_DEFAULTSET), MSG_STR, MB_OKCANCEL) != IDOK)
					return	TRUE;
...
void TSendDlg::GetOrder(void)
{
	...
	if (hostListView.SendMessage(LVM_GETCOLUMNORDERARRAY, maxItems, (LPARAM)order) == FALSE) {
		MessageBox(GetLoadStr(IDS_COMCTL), GetLoadStr(IDS_CANTGETORDER));
		return;
	}
	for (int cnt=0; cnt < MAX_SENDWIDTH; cnt++) {
		if (GetItem(ColumnItems, FullOrder[cnt]))
...
BOOL TSendDlg::EventInitMenu(UINT uMsg, HMENU hMenu, UINT uPos, BOOL fSystemMenu)
{
	...
	case WM_INITMENU:
		{
			ModifyMenuU8(hMenu, MENU_SAVEPOS, MF_BYCOMMAND|(cfg->SendSavePos ? MF_CHECKED :  0), MENU_SAVEPOS, GetLoadStr(IDS_SAVEPOS));
		}
		return	TRUE;
	}
	return	FALSE;
...
BOOL TSendDlg::EvTimer(WPARAM _timerID, TIMERPROC proc)
{
	...
	*buf = 0;
	...
	strcat(buf, GetLoadStr(IDS_RETRYSEND));
	int ret = MessageBox(buf, MSG_STR, MB_RETRYCANCEL|MB_ICONINFORMATION);
	delete [] buf;
...
void TSendDlg::DisplayMemberCnt(void)
{
	...
	if (*buf == 0) {
		strcpy(buf, GetLoadStr(IDS_USERNUM));
	}
...
BOOL TSendDlg::SendMsg(void)
{
	...
	if (listOperateCnt) {
		MessageBox(GetLoadStr(IDS_BUSYMSG));
		return	FALSE;
	}
...
//	char	*appendStr = selectNum > 0 ? GetLoadStr(IDS_MOVETO) : GetLoadStr(IDS_SELECT);
	char	*appendStr = selectNum > 0 ? GetLoadStr(IDS_MOVETO) : GetLoadStr(IDS_MOVETO);
...
// priority menu
	for (cnt=cfg->PriorityMax; cnt >= 0; cnt--)
	{
		...
		if (cnt == 0)
			ptr += wsprintf(ptr, GetLoadStr(IDS_NODISP));
		else if (cnt == 1)
			ptr += wsprintf(ptr, GetLoadStr(IDS_DEFAULTDISP));
		else
			ptr += wsprintf(ptr, GetLoadStr(IDS_DISPPRIORITY), cfg->PriorityMax - cnt + 1);

		if (isJapanese)
			ptr += wsprintf(ptr, " %s ", appendStr);

		wsprintf(ptr, cnt == 1 ? GetLoadStr(IDS_MEMBERCOUNTDEF) : GetLoadStr(IDS_MEMBERCOUNT), hosts->PriorityHostCnt(cnt * PRIORITY_OFFSET, PRIORITY_OFFSET), cfg->priorityHosts.PriorityHostCnt(cnt * PRIORITY_OFFSET, PRIORITY_OFFSET));
	...
	AppendMenuU8(hPriorityMenu, MF_STRING|(hiddenDisp ? MF_CHECKED : 0), MENU_PRIORITY_HIDDEN, GetLoadStr(IDS_TMPNODISPDISP));
	AppendMenuU8(hPriorityMenu, MF_STRING, MENU_PRIORITY_RESET, GetLoadStr(IDS_RESETPRIORITY));
	AppendMenuU8(hMenu, MF_POPUP, (UINT)hPriorityMenu, GetLoadStr(IDS_SORTFILTER));
...
// group select
	...
	AppendMenuU8(hMenu, MF_POPUP|(::GetMenuItemCount(hGroupMenu) ? 0 : MF_GRAYED), (UINT)hGroupMenu, GetLoadStr(IDS_GROUPSELECT));
//	AppendMenuU8(hMenu, MF_SEPARATOR, 0, 0);
...
{
	AppendMenuU8(hMenu, MF_STRING, MENU_FINDDLG, GetLoadStr(IDS_FINDDLG));
	AppendMenuU8(hMenu, MF_STRING, MENU_FILEADD, GetLoadStr(IDS_FILEATTACHMENU));
	AppendMenuU8(hMenu, MF_STRING, MENU_FOLDERADD, GetLoadStr(IDS_FOLDERATTACHMENU));
	AppendMenuU8(hMenu, MF_SEPARATOR, 0, 0);
	AppendMenuU8(hMenu, MF_STRING, MENU_SAVECOLUMN, GetLoadStr(IDS_SAVECOLUMN));

	AppendMenuU8(hMenu, MF_POPUP, (UINT)::LoadMenu(TApp::GetInstance(), (LPCSTR)SENDFONT_MENU), GetLoadStr(IDS_FONTSET));
	AppendMenuU8(hMenu, MF_POPUP, (UINT)::LoadMenu(TApp::GetInstance(), (LPCSTR)SIZE_MENU), GetLoadStr(IDS_SIZESET));
	AppendMenuU8(hMenu, MF_STRING, MENU_SAVEPOS, GetLoadStr(IDS_SAVEPOS));
	AppendMenuU8(hMenu, MF_STRING, MENU_MEMBERDISP, GetLoadStr(IDS_MEMBERDISP));
}
