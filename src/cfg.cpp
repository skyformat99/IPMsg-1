...
#define BALLONNOTIFY_STR	"BallonNotify"
...
Cfg::Cfg(ULONG _nicAddr, int _portNo)
{
	...
	for (i=0; abs_ids[i] && i < sizeof(DefaultAbsence) / sizeof(char *); i++) {
		DefaultAbsence[i] = GetLoadStr(abs_ids[i]);
	}
	...
	for (i=0; absh_ids[i] && i < sizeof(DefaultAbsenceHead) / sizeof(char *); i++) {
		DefaultAbsenceHead[i] = GetLoadStr(absh_ids[i]);
	}
}
...
// Find User
	...
	*PasswordStr = 0;
	...
	reg.GetInt(BALLONNOTIFY_STR, &BallonNotify);
...
BOOL Cfg::WriteRegistry(int ctl_flg)
{
	...
	if (ctl_flg & CFG_GENERAL)
	{
		...
		reg.SetInt(BALLONNOTIFY_STR, BallonNotify);
