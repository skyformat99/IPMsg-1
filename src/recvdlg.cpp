...
BOOL TRecvDlg::EvCreate(LPARAM lParam)
{
	...
	if (msg.command & IPMSG_BROADCASTOPT)
		SetDlgItemText(RECV_TITLE, GetLoadStr(IDS_BROADCAST));
	else if (msg.command & IPMSG_MULTICASTOPT)
		SetDlgItemText(RECV_TITLE, GetLoadStr(IDS_MULTICAST));
	else
		SetDlgItemText(RECV_TITLE, GetLoadStr(IDS_UNICAST));

	if (msg.command & IPMSG_AUTORETOPT)
		SetDlgItemText(RECV_TITLE, GetLoadStr(IDS_UNIABSENCE));
	...
	if (msg.command & IPMSG_FILEATTACHOPT)
	{
		...
		strcat(buf, " ");
		strcat(buf, GetLoadStr(IDS_FILEATTACH));
		SetDlgItemText(OPEN_BUTTON, buf);
	}
	if (msg.command & IPMSG_PASSWORDOPT)
	{
		GetDlgItemText(OPEN_BUTTON, buf, sizeof(buf));
		strcat(buf, GetLoadStr(IDS_KEYOPEN));
		SetDlgItemText(OPEN_BUTTON, buf);
	}
	...
	if (cfg->AbnormalButton)
		SetDlgItemText(IDOK, GetLoadStr(IDS_INTERCEPT));
...
BOOL TRecvDlg::EvCommand(WORD wNotifyCode, WORD wID, LPARAM hWndCtl)
{
	...
	case IDOK:
		if (openFlg && ::IsDlgButtonChecked(hWnd, QUOTE_CHECK)) {
			...
			for (int i=0, j=0; buf.Buf()[i] && j < MAX_UDPBUF; i++, j++) {
	...
	case FILE_BUTTON:
		if (fileObj && fileObj->conInfo)
		{
			...
			int ret = MessageBox(GetLoadStr(IDS_TRANSSTOP), MSG_STR, MB_OKCANCEL);
			if (fileObj->hThread)
...
BOOL TRecvDlg::SaveFile(void)
{
	...
	if (ConnectRecvFile())
		SetDlgItemText(FILE_BUTTON, GetLoadStr(IDS_PREPARETRANS));
	else {
		delete fileObj->conInfo;
...
BOOL TRecvDlg::EventInitMenu(UINT uMsg, HMENU hMenu, UINT uPos, BOOL fSystemMenu)
{
	...
	case WM_INITMENU:
		{
			ModifyMenuU8(hMenu, MENU_SAVEPOS, MF_BYCOMMAND|(cfg->RecvSavePos ? MF_CHECKED :  0), MENU_SAVEPOS, GetLoadStr(IDS_SAVEPOS));
		}
		return	TRUE;
	}
	return	FALSE;
...
{
	AppendMenuU8(hMenu, MF_POPUP, (UINT)::LoadMenu(TApp::GetInstance(), (LPCSTR)RECVFONT_MENU), GetLoadStr(IDS_FONTSET));
...
BOOL TRecvDlg::DecodeDirEntry(char *buf, FileInfo *info, char *u8fname)
{
	...
	while ((ptr = strchr(tok, '?')) != NULL)	// UNICODE までの暫定
		*ptr = '_';
...
//		case IPMSG_FILE_CREATETIME:	info->SetCrtime(val); break;	現状では使わない
//		case IPMSG_FILE_ATIME:		info->SetAtime(val); break;		現状では使わない
		default: break;
		}
	}
	return	TRUE;
		...
		if (fileObj->offset == fileObj->infoLen)
		{
			...
			if (GET_MODE(fileObj->curFileInfo.Attr()) == IPMSG_FILE_DIR)
			{
				...
				if (MakePath(buf, fileObj->path, fname) >= MAX_PATH_U8)
					return	MessageBox(buf, GetLoadStr(IDS_PATHTOOLONG)), FALSE;
...
BOOL TRecvDlg::OpenRecvFile(void)
{
	...
	if (MakePath(path, fileObj->isDir ? fileObj->path : fileObj->saveDir, fileObj->curFileInfo.Fname()) >= MAX_PATH_U8)
		return	MessageBox(path, GetLoadStr(IDS_PATHTOOLONG)), FALSE;
	if (IsSafePath(path, fileObj->curFileInfo.Fname()) == FALSE)
		return	MessageBox(path, GetLoadStr(IDS_NOTSAFEPATH)), FALSE;
	...
	if ((fileObj->hFile = CreateFileU8(path, GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0)) == INVALID_HANDLE_VALUE)
		return	fileObj->isDir ? FALSE : (MessageBox(GetLoadStr(IDS_CREATEFAIL), path), FALSE);
...
BOOL TRecvDlg::RecvFile(void)
{
	...
	if (fileObj->offset + size >= fileObj->curFileInfo.Size() || cfg->TransMax == wresid)
	{
		...
		if (::WriteFile(fileObj->hFile, fileObj->recvBuf, wresid, &wsize, 0) != TRUE || wresid != (int)wsize)
			return	MessageBox(GetLoadStr(IDS_WRITEFAIL)), FALSE;
